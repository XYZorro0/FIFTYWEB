...
// self-check parses.
// forked from design-sync lib/bundle.mjs — Next.js components reference process.* globals not covered by NODE_ENV only
...
export async function bundleToIife({ entry, globalName, nodePaths, out, tsconfig }) {
  ...
  try {
    buildResult = await build({
      ...
      // Inject a process shim so Next.js components that reference process.env.*
      // and __NEXT_IMAGE_OPTS at runtime (not compile-time) don't crash in the browser.
      banner: { js: `if(typeof process==="undefined"){window.process={env:{NODE_ENV:"development",__NEXT_IMAGE_OPTS:{deviceSizes:[640,750,828,1080,1200,1920],imageSizes:[16,32,48,64,96,128,256,384],path:"/_next/image",loader:"default",dangerouslyAllowSVG:false,unoptimized:true},NEXT_RUNTIME:"browser"},platform:"browser"};}else{process.env.__NEXT_IMAGE_OPTS=process.env.__NEXT_IMAGE_OPTS||{deviceSizes:[640,750,828,1080,1200,1920],imageSizes:[16,32,48,64,96,128,256,384],path:"/_next/image",loader:"default",dangerouslyAllowSVG:false,unoptimized:true};}` },
      outfile: bundleJs,
      logLevel: 'warning',
